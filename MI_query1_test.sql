...
FROM   pbi.perform_so AL1,
       ...
       pbi.perform_table_x_district_info_lookup AL10
WHERE  ( AL2.itm_cd = AL3.itm_cd
         ...
         AND AL9.site_id = AL10.x_district )
       AND ( Year(Cast(AL1.final_dt AS DATE)) >= Year(Cast(Getdate() AS DATE))
             AND Cast(AL1.final_dt AS DATE) <= Cast(Dateadd(wk, Datediff(wk, 6,
                                                                Getdate
                                                                ()),
                                                    7
                                                    ) AS
...
'PTAC HP','AIR HANDLER', 'ATMOSPHERIC BURNER', 'AUXILIARY',
             ...
             AND AL7.work_group IN ( 'Cleaner', 'Helpers', 'HIM', 'Installers',
                                     'Service Technician' ) 
